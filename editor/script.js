...
class BlockEditor {
    ...
    createEquationBlock(blockElement, block) {
        ...
        editorDiv.textContent = block.content || '';
        
        // Make editor active by default for new blocks
        if (!block.content) {
        ...
        // Hide viewer initially for new blocks
        if (!block.content) {
        ...
        // Set initial button state for new blocks
        if (!block.content) {
        ...
        if (block.content) {
            try {
                katex.render(block.content, viewerDiv, katexOptions);
            } catch (e) {
        ...
        viewerDiv.addEventListener('click', (e) => {
            ...
            if (viewerDiv.classList.contains('selected')) {
                ...
                const handleClickOutside = (e) => {
                    if (!viewerDiv.contains(e.target)) {
